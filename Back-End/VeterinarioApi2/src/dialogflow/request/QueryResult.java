...
import restAccess.DialogflowWebhook;
		...
		for (Map<String, Object> map: fulfillmentMessages) {
			for (String key: map.keySet()) {
				if (key.equalsIgnoreCase("text")){
					try {
						this.fulfillmentMessages.add(DialogflowWebhook.mapper.readValue(DialogflowWebhook.mapper.writeValueAsString(map.get(key)), Text.class));
					} catch (IOException e) {
